...
在大语言模型（LLM）中，**偏见**（Bias）是指模型在生成内容时表现出对某些群体、概念或观点的系统性不公平倾向。这种偏见并非模型“主观故意”，而是由训练数据和模型架构的特性共同导致的。
...
- **互联网文本天然带有偏见**：网络上大量内容反映了现实世界中的性别、种族、宗教、阶级等刻板印象。
  - 例如：大量文本将“护士”与“女性”关联，将“程序员”与“男性”关联。
- **历史不平等的体现**：过去的社会结构和话语体系中存在系统性歧视，这些被记录在数据中。
  - 如：对某些种族的负面描述、对女性职业的限制性语言。
...
### 2. **数据分布不均衡**
- 某些群体（如白人男性）在训练数据中出现频率远高于其他群体（如少数族裔、LGBTQ+、残障人士）。
- 模型对高频群体更“熟悉”，生成内容更自然；对低频群体则可能产生刻板印象或错误。

> 📌 举例：当问“医生是谁？”时，模型更可能生成“男性”形象。

---

### 3. **语言结构与文化隐含偏见**
- 某些语言本身包含性别化词汇（如法语、德语的名词性别）。
- 中文虽无语法性别，但“他”常被泛化为“所有人”，造成“男性中心”语境。
...
### 4. **模型架构与优化目标**
- 模型的目标是“预测下一个词”，而不是“公平”或“中立”。
- 为了最大化预测准确率，模型倾向于选择“常见搭配”，即使这些搭配带有偏见。

> 例如：输入“她是一位科学家，她很……” → 模型可能倾向接“温柔”而非“权威”。

---

### 5. **评估与反馈机制的偏见**
- 如果模型的微调或强化学习（RLHF）使用了带有偏见的人类反馈（Human Feedback），也会引入偏见。
- 例如：人类标注者可能认为“男性更适合领导岗位”，从而给相关回答更高评分。

---

### 6. **提示词（Prompt）的放大效应**
- 即使模型本身无偏见，用户输入的提示词也可能触发偏见。
  - 如：“请用女性口吻写一封辞职信” → 模型可能默认“女性更情绪化”。
- 模型会根据上下文“补全”预期角色，从而暴露偏见。
...
| 方法 | 说明 |
|------|------|
| **去偏训练数据** | 清洗、平衡数据分布，减少偏见样本 |
| **去偏算法** | 如对抗训练、公平性正则化 |
| **后处理过滤** | 生成后检测并修正偏见输出 |
| **提示工程** | 使用中性、包容性提示词 |
| **人类评估 + 多样性反馈** | 引入多元视角进行模型调优 |
...
**A: Decoder-only模型**通过一个上下文嵌入函数 $\phi$ 来将序列的前 $i−1$ 个词 $x_{1:i−1}$ 映射到一个嵌入向量 $\phi(x_{1:i−1})$ ；随后应用嵌入矩阵 $E$ 和 $softmax$ 函数来得到第 $i$ 个词 $x_i$ 的概率分布 $p(x_{i}|x_{1:i})$ 。那么，Decoder-only模型训练的条件分布可以表示为 $p(x_{i}|x_{1:i})=softmax(E\phi(x_{1:i-1})_{i-1})$。
...
Q：为什么现在的LLM都是Decoder-only的架构？  
A：1、因为Encoder的双向attention的注意力矩阵容易退化为低秩状态，而casual attention的注意力矩阵是下三角矩阵，必然是满秩的，建模能力更强；就生成问题而言，引入双向注意力并无好处。Encoder-Decoder架构在某些场景下表现更好，主要是因为多了一倍参数，同等参数量、同等推理成本下，Decoder-only架构是最好的选择；
2、Decoder-only架构+next token prediction预训练，每个位置能接触的信息比其他架构少，要预测下一个token难度高，当模型足够大，数据足够多的时候，Decoder-only的模型学习通用表征的上限更高；  
3、上下文学习为Decoder-only架构带来了更好的few-shot性能，prompt和demonstration的信息可以视为对模型参数的隐式微调，Decoder-only的架构相比Encoder-Decoder在in-context learning上会更有优势，因为prompt可以更加直接地作用于decoder每一层的参数，微调的信号更强；  
4、causal attention （Decoder-only的单向attention）具有隐式的位置编码功能，打破了transformer的位置不变性，而带有双向attention的模型，如果不带位置编码，双向attention的部分token可以对换也不改变表示，对语序的区分能力天生较弱。  
5、推理效率高，支持多轮对话：Decoder-only架构支持KV-Cache复用，对多轮对话更友好。  
